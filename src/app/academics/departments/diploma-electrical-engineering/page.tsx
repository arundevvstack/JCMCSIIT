...
export const metadata: Metadata = {
  title: 'Diploma in Electrical Engineering | JCMCSIIT',
  description: 'AICTE-approved Diploma in Electrical Engineering at JCMCSIIT. Focus on practical skills and career readiness.',
};

export default function DiplomaEEEPage() {
  const data = departmentsData["diploma-electrical-engineering"];
  // Pulling the main EEE department data to inherit its vision and mission as requested
  const mainEeeData = departmentsData["eee"];
  const eeeFaculty = facultyData.filter((f) => f.department_id === 'eee');
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="max-w-4xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-slate-900 mb-6">
              {data?.name || 'Diploma in Electrical Engineering'}
            </h1>
            <p className="text-xl md:text-2xl text-slate-600 leading-relaxed mb-8">
              {data?.shortDescription || 'Electrical machines, power systems, industrial automation, and renewable energy technologies.'}
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="space-y-8">
              <div>
                ...
                <p className="text-lg text-slate-600 leading-relaxed">
                  {data?.fullDescription || 'The diploma program focuses on building practical engineering skills, innovation, and career readiness in Electrical Engineering.'}
                </p>
              </div>

              {/* Vision and Mission (Inherited from EEE) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
                <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100 shadow-sm">
                  ...
                  <p className="text-slate-600 text-sm leading-relaxed">
                    {mainEeeData?.vision || 'Vision statement will be updated.'}
                  </p>
                </div>
                <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100 shadow-sm">
                  <ShieldCheck className="w-8 h-8 text-primary mb-4" />
                  <h3 className="text-xl font-bold text-slate-900 mb-3">Mission</h3>
                  {Array.isArray(mainEeeData?.mission) ? (
                    <ul className="text-slate-600 text-sm leading-relaxed space-y-2 list-disc pl-4">
                      {mainEeeData.mission.map((item, idx) => (
                        <li key={idx}>{item}</li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-slate-600 text-sm leading-relaxed">
                      {mainEeeData?.mission || 'Mission statement will be updated.'}
                    </p>
                  )}
                </div>
              </div>
